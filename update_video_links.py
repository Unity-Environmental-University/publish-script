...
def advanced_options_ui(root, course_id, source_course_id):
    ...
    for option in advanced_options:
        button = tk.Button(master=win, text=option['name'],
                           command=lambda: execute_option(win, course_id, source_course_id, option))
        button.pack()
...
def create_missing_assignments(course_id, source_course_id):
    ...
    for source_module in source_modules:
        ...
        dest_module = find_new_module_by_number(number, modules)
    ...
    if len(to_remove) and tk.messagebox.askyesno(
        ...
        for dest_module in to_remove:
            remove_module(course_id, dest_module, True)
...
def remove_assignments_and_discussions_not_in_modules(course_id):
    ...
    if len(discussions_to_delete) > 0 and tk.messagebox.askyesno(
        ...
        for discussion in discussions_to_delete:
            result = requests.delete(f"{API_URL}/courses/{course_id}/discussion_topics/{discussion['id']}",
                                     headers=headers)
            print(result)
...
def update_weekly_overviews(course_id, source_course_id):
    ...
    for source_module in source_modules:
        ...
        if "week" in title_words[0].lower():
            ...
            source_overview_page_info = next(
                filter(lambda item: "overview" in item["title"].lower() and item["type"] == "Page", items))
...
def set_course_grad(course_id):
    ...
    grad_standard = next(filter(lambda scheme: GRAD_SCHEME_NAME.lower() in scheme['title'].lower(), grading_standards),
                         None)
...
def update_syllabus(course_id, syllabus_banner_url, term, dates, learning_objectives_section, description_section,
...
def update_course_overview(course_id, source_course_id, learning_objectives_section, description_section,
                           textbook_section):
    # update overview
    modules = get_modules(course_id)
...
def get_strategy_until_header_like(soup, header_pattern):
    ...
    while el and el.name != "h4" and not len(list(el.find_all('strong', string=re.compile(r':')))) > 0 and not len(
            el.text) < 5:
        print(el.name)
...
def update_learning_materials(course_id: str, source_course_id: str, start_index: int = 1, end_index: int = 8,
    ...
    for i in range(start_index, end_index + 1):
        ...
        if source_header:
            ...
            learning_materials = get_section(source_soup,
                                             re.compile("Please read (and watch )?the following materials:",
                                                        re.IGNORECASE))
